<script>
  ...
  let mp = $derived(data.images.flatMap((e)=>{

    return e.images.map((g)=>{
      let copy = {...e}
      delete copy['images']
      return {...copy, image:g}
    })
...
// ] 
  ...
  const carouselImages = $derived([...mp].splice(0, 40));
...
<div class="container mt-12 flex flex-col gap-12">
...
<div class="carousel-container mb-12">
  {#each carouselImages as image, index}
    <div
      ...
      style="background-image: url('https://zaria.pockethost.io/api/files/gallery/{image.id}/{image.image}'); background-size: cover; background-position: center;"
    ></div>
  {/each}
</div>
...
<div class="masonry-grid px-4">
  {#each paginatedImages as image}
    <div class="masonry-item">
      <img
        src={`https://zaria.pockethost.io/api/files/gallery/${image.id}/${image.image}`}
        alt="{image.info}"
